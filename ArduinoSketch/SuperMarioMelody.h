...
//return next delay between two notes
int moveToNextStepSong(float speed) {
  
  //no not allow overflow
  if(thisNote >= melodySize) {
     thisNote = 0; 
  }
  ...
  //move to next note in next cycle
  thisNote++;
  return noteDuration * speed;    
}
